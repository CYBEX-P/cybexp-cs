...
def archive_one(event, cache_coll, fs, pkey_fp, parsemain):
    try:
        ...
        instance = parsemain(typtag, orgid, timezone, data)

        cache_coll.update_one({"_id" : event["_id"]}, {"$set":{"processed":True}})
    except: logging.error("proc.archive.archive_one: -- ", exc_info=True)
...
def archive(config):
    ...
    while True:
        try:
            ...
            for e in cursor: archive_one(e, cache_coll, fs, private_key_file_path, parsemain)
            n_failed_attempts = 0
